...
def order_history(request, template="shop/order_history.html"):
    ...
    for i in all_orders:
        ...
        for item in i.items.all():
            product=ProductVariation.objects.filter(sku=item.sku)
            # print "hehe"
            product_item={"order_item":item,"product":product[0]}
            order["products"].append(product_item)
        orders.append(order)
        order={}
    print orders
    #############
    ...
    # except EmptyPage:
    #     orders=paginator.page(paginator.page)
