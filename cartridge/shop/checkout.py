...
from cartridge.shop.payment import eway
...
def default_payment_handler(request, order_form, order):
    ...
    is unsuccessful.
    """
    dict =eway.process_direct_payment(request, order_form, order)
    print dict
    dict=dict.get('ewayResponse')
    print dict.get('ewayTrxnStatus')
    if dict.get('ewayTrxnStatus')=='True':
        print "hehe"
        return dict.get('ewayTrxnNumber')
    else:
        raise CheckoutError(_("Transaction failed"))
