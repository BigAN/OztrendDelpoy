...
class OrderForm(FormsetForm, DiscountForm):
    ...
    same_billing_shipping = forms.BooleanField(required=False, initial=True,
        label=_("My billing details are the same as my delivery details"))
    ...
    def preprocess(cls, data):
        ...
        if data.get("same_billing_shipping", "") == "on":
            for field in data:
                ship_field = field.replace("billing_detail","shipping_detail")
                if field.startswith("billing_detail") and ship_field in data:
                    data[field] = data[ship_field]
        return data
